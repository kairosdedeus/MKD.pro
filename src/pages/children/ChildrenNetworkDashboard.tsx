...
import { ChildrenNetworkIcon } from "@/components/shared/MinistryIcons";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function buildWhatsAppText(
  ...
  const lines = [
    `ESCALA REDE DE CRIANÇAS - ${monthName} / ${year}`,
    "",
    teamName,
  ];
...
export function ChildrenNetworkDashboard() {
  ...
  const childrenTeam = teams.find(
    (team) => team.team_type?.codigo === "rede_criancas",
  );
  const {
    schedules,
    loading: loadingSchedules,
    refetch,
  } = useSchedules(childrenTeam?.id || "", currentMonth);
  ...
  const canManage =
    !!user &&
    (isGerencial(profiles || []) || isLeader(profiles || [], "rede_criancas"));
  ...
  const whatsAppText = buildWhatsAppText(
    schedules,
    currentMonth,
    childrenTeam?.nome || "Rede de Crianças",
  );
  ...
  if (!childrenTeam) {
    return (
      <EmptyState
        icon={Users}
        title="Equipe da Rede de Crianças não encontrada"
        description="Crie uma equipe do tipo Rede de Crianças em Gerencial -> Equipes."
      />
    );
  }
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-foreground">
            <ChildrenNetworkIcon className="h-9 w-9" />
            Rede de Crianças
          </h1>
          <p className="text-xs text-muted-foreground">{childrenTeam.nome}</p>
        </div>
        {canManage && (
          <Button
            variant="outline"
      ...
      <div className="space-y-3 lg:grid lg:grid-cols-[1fr_240px] lg:items-start lg:gap-4 lg:space-y-0">
                                ...
                                >
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <div className="mt-0.5 flex items-center gap-2">
                                      ...
                                      <span className="text-xs text-muted-foreground">
                                        {schedule.members?.length || 0} crianças
                                      </span>
                                    </div>
                                  </div>
                                  {canManage && (
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card lg:sticky lg:top-4">
          <div className="flex items-center gap-3 border-b border-border px-4 py-3">
            ...
            <span className="text-xs text-muted-foreground">
              {childrenTeam.members?.length || 0}
            </span>
          </div>
          <div className="p-3">
            {!childrenTeam.members || childrenTeam.members.length === 0 ? (
              <p className="py-3 text-center text-sm text-muted-foreground">
                Nenhum membro
              </p>
            ) : (
              <div className="space-y-1">
                {childrenTeam.members.map((member) => (
                  <div
                    key={member.id}
