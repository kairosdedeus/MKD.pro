...
import { Plus, Users as UsersIcon, Pencil, Trash2, MoreVertical, PowerOff } from 'lucide-react'
...
export function TeamsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <span className="text-sm font-normal text-gray-400">{teams.length} equipe(s)</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {teams.length > 0 ? (
                  ...
                  <TableRow key={team.id}>
                    ...
                    <TableCell>
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                        MINISTRY_COLORS[(team as any).team_type?.codigo] || 'bg-gray-100 text-gray-700'
                      }`}>
                        ...
                        {(team as any).leader?.nome ? (
                          <>
                            <div className="w-6 h-6 rounded-full bg-purple-100 flex items-center justify-center text-xs font-semibold text-purple-700">
                              {(team as any).leader.nome.charAt(0).toUpperCase()}
                            </div>
                            <span className="text-sm">{(team as any).leader.nome}</span>
                          </>
            ...
            <EmptyState
              ...
              action={{ label: 'Nova Equipe', onClick: () => setCreateModalOpen(true) }}
            />
          )}
        </CardContent>
      </Card>

      <CreateTeamModal open={createModalOpen} onOpenChange={setCreateModalOpen} onSuccess={refetch} />
      ...
      <AlertDialog open={!!deactivatingTeam} onOpenChange={o => !o && setDeactivatingTeam(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Desativar equipe?</AlertDialogTitle>
            <AlertDialogDescription>
              A equipe <strong>"{deactivatingTeam?.nome}"</strong> será desativada e não aparecerá mais nos dashboards.
              As escalas existentes serão mantidas.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deactivating}>Cancelar</AlertDialogCancel>
            <AlertDialogAction className="bg-red-600 hover:bg-red-700" onClick={handleDeactivate} disabled={deactivating}>
