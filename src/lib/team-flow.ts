...
type TeamTypeIcon = ComponentType<SVGProps<SVGSVGElement>>;
...
export const TEAM_TYPE_ICONS: Record<string, TeamTypeIcon> = {
  louvor: WorshipIcon,
  danca: DanceIcon,
  midia: MediaIcon,
  obreiros: HandHeart,
  ...
  return Array.from(
    new Set(
      selectedProfileIds
        .map(
          (profileId) =>
            profiles.find((profile) => profile.id === profileId)?.codigo,
        )
