...
import { getAvailableGeneratedEmail, isDuplicateGeneratedEmailError } from '@/lib/user-email'
...
export function CreateUserModal({ open, onOpenChange, onSuccess }: CreateUserModalProps) {
  ...
  const onSubmit = async (data: UserFormData) => {
    ...
    try {
      const availableEmail = await getAvailableGeneratedEmail(data.nome, data.sobrenome)
      if (!availableEmail) {
        toast({ variant: 'destructive', title: 'Nao foi possivel gerar o login' })
        return
      }
      ...
      if (selectedTeamIds.length > 0 && result?.id) {
        const { error: memberError } = await supabase
          .from('team_members')
          .upsert(
            selectedTeamIds.map(teamId => ({ team_id: teamId, user_id: result.id, ativo: true })),
      ...
      const teamNames = selectedTeamIds
        .map(teamId => (teams as any[]).find((team: any) => team.id === teamId)?.nome)
        .filter(Boolean)

      toast({
        title: 'Usuario criado!',
        description: teamNames.length > 0
      ...
      if (isDuplicateGeneratedEmailError(error)) {
        toast({
          ...
          description: 'Tente novamente. O sistema vai sugerir automaticamente o proximo login disponivel.',
        })
        if (nome && sobrenome) {
          const nextEmail = await getAvailableGeneratedEmail(nome, sobrenome).catch(() => '')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Crie um novo usuario no sistema</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="password" type="password" placeholder="Minimo 8 caracteres" autoComplete="new-password" {...register('password')} />
