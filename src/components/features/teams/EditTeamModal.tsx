...
export function EditTeamModal({ open, onOpenChange, team, onSuccess }: EditTeamModalProps) {
  ...
  useEffect(() => {
    ...
    if ((team as any).team_type_id) {
      teamService.getTeamFunctions((team as any).team_type_id).then(setTeamFunctions).catch(() => {})
    }
  }, [team, open])
  ...
  const toggleMember = (user: { id: string; nome: string }) => {
    ...
    if (exists) {
      ...
      setMembers(prev => [...prev, { userId: user.id, nome: user.nome, functionIds: [], expanded: false, isNew: true }])
    }
  }
  ...
  const toggleFunction = (userId: string, fnId: string) => {
    setMembers(prev => prev.map(m => {
      ...
      return { ...m, functionIds: has ? m.functionIds.filter(id => id !== fnId) : [...m.functionIds, fnId] }
    }))
  }

  const toggleExpand = (userId: string) => {
    setMembers(prev => prev.map(m => m.userId === userId ? { ...m, expanded: !m.expanded } : m))
  }
  ...
  useEffect(() => {
    ...
    setMembers(prev => {
      ...
      return [...prev, { userId: leaderId, nome: leaderUser.nome, functionIds: [], expanded: false, isNew: true }]
    })
  }, [leaderId])
  ...
  const handleSave = async () => {
    ...
    if (!leaderId) { toast({ variant: 'destructive', title: 'Selecione um líder' }); return }
    ...
    try {
      ...
      for (const userId of toRemove) {
        ...
        if (tm) {
          await (supabase as any).from('team_member_functions').delete().eq('team_member_id', tm.id)
  ...
  if (loadingUsers) {
    return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><LoadingSpinner /></DialogContent></Dialog>
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">✏️ Editar Equipe</DialogTitle>
          <DialogDescription>Atualize os dados, membros e funções da equipe</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label>Membros</Label>
              <span className="text-xs text-gray-400">{members.length} selecionado(s)</span>
            </div>
                  ...
                  <div key={user.id} className={`p-3 ${isLeader ? 'bg-purple-50' : ''}`}>
                    <div className="flex items-center gap-3">
                      {isLeader ? (
                        ...
                        <Checkbox checked={isSelected} onCheckedChange={() => toggleMember(user)} />
                      )}
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {isLeader && <span className="text-xs bg-purple-600 text-white px-1.5 py-0.5 rounded-full">Líder</span>}
                        </div>
                        {isSelected && member!.functionIds.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1 ml-9">
                            {member!.functionIds.map(fId => {
                              ...
                              return fn ? (
                                <span key={fId} className="text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded">{fn.nome}</span>
                              ) : null
                            })}
                          </div>
                        )}
                      ...
                      {isSelected && teamFunctions.length > 0 && (
                        <button type="button" onClick={() => toggleExpand(user.id)} className="text-gray-400 hover:text-gray-600">
                          {member!.expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                        </button>
                      )}
                    </div>
                      ...
                      <div className="mt-2 ml-10 p-2 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {teamFunctions.map(fn => (
                            <label key={fn.id} className="flex items-center gap-1.5 text-sm cursor-pointer">
                              <Checkbox
                                checked={member!.functionIds.includes(fn.id)}
                                onCheckedChange={() => toggleFunction(user.id, fn.id)}
                              />
            ...
            {teamFunctions.length > 0 && (
              <p className="text-xs text-gray-400">💡 Clique na seta ▼ para atribuir funções ao membro</p>
            )}
          </div>
        </div>
