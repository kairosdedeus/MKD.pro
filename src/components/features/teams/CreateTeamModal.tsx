...
export function CreateTeamModal({ open, onOpenChange, onSuccess }: CreateTeamModalProps) {
  ...
  const toggleMember = (user: { id: string; nome: string }) => {
    ...
    if (exists) {
      ...
      setMembers([...members, { userId: user.id, nome: user.nome, functionIds: [], expanded: false }])
    }
  }
  ...
  const toggleMemberFunction = (userId: string, fnId: string) => {
    setMembers(members.map(m => {
      ...
      return {
        ...m,
        functionIds: has ? m.functionIds.filter(id => id !== fnId) : [...m.functionIds, fnId],
      }
    }))
  }
  ...
  const handleSubmit = async () => {
    if (!nome.trim()) {
      toast({ variant: 'destructive', title: 'Nome obrigatório', description: 'Informe o nome da equipe.' })
      return
    }
    if (!teamTypeId) {
    ...
    try {
      ...
      if (teamFunctions.length > 0 && members.some(m => m.functionIds.length > 0)) {
        // Buscar os team_members criados
        const { data: teamMembers } = await import('@/lib/supabaseClient').then(({ supabase }) =>
          supabase.from('team_members').select('id, user_id').eq('team_id', team.id)
        )

        if (teamMembers) {
          for (const member of members) {
            if (member.functionIds.length === 0) continue
            const tm = teamMembers.find((tm: any) => tm.user_id === member.userId)
            if (tm) {
  ...
  const colorClass = selectedType ? (MINISTRY_COLORS[selectedType.codigo] || '') : ''
