...
export function WorshipFixedTeamModal({
  ...
  const sortedMembers = [...members].sort((a, b) =>
    (a.user?.nome || '').localeCompare(b.user?.nome || '', 'pt-BR', { sensitivity: 'base' })
  )
  ...
  const toggleMember = (memberId: string) => {
    setSelectedMembers(prev => {
      ...
      const defaultFunctionId = member?.functions?.length === 1 ? member.functions[0].id : undefined
  ...
  const handleSubmit = async () => {
    if (!name.trim()) {
      toast({ variant: 'destructive', title: 'Informe o nome da equipe padrão' })
      return
    }

    if (selectedMembers.length === 0) {
      toast({ variant: 'destructive', title: 'Selecione pelo menos um membro' })
      return
    }

    const membersWithoutFunction = selectedMembers.filter(member => member.function_ids.length === 0)
    if (membersWithoutFunction.length > 0) {
      toast({ variant: 'destructive', title: 'Defina a função de todos os membros' })
      return
    }

    try {
      setSaving(true)
      const payload: WorshipFixedTeamMemberInput[] = selectedMembers.map(member => ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-1rem)] max-w-3xl max-h-[92vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Editar equipe padrão' : 'Nova equipe padrão'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <Label className="text-base font-semibold">Membros e funções</Label>
            ...
            {members.length === 0 ? (
              <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
                ...
                Adicione membros na equipe de Louvor antes de criar uma equipe padrão.
              </div>
            ) : (
              <div className="space-y-2">
                {sortedMembers.map(member => {
                  const selected = selectedMembers.find(item => item.team_member_id === member.id)
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-lg border p-3 transition-colors ${
                        selected ? 'border-purple-300 bg-purple-50' : 'border-border bg-background'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{member.user?.nome}</p>
                          {member.functions && member.functions.length > 0 && (
                            <p className="text-xs text-muted-foreground break-words">
                              Funções cadastradas: {member.functions.map(fn => fn.nome).join(', ')}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      {selected && (
                        <div className="mt-3 pl-0 sm:pl-12">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {functions.map(fn => {
                              const active = selected.function_ids.includes(fn.id)
                              return (
                                <button
                                  key={fn.id}
                                  type="button"
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button onClick={handleSubmit} disabled={saving} className="w-full bg-purple-600 hover:bg-purple-700 sm:w-auto">
            {saving ? 'Salvando...' : isEditing ? 'Salvar alterações' : 'Criar equipe'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
