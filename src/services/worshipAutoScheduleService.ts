...
function getTeamCode(value: string) {
  ...
  return REQUIRED_PRESET_CODES.find(code => normalized.includes(normalizeText(code)))
}

function getFunction(functions: TeamFunction[], name: string) {
  const fn = functions.find(item => normalizeText(item.nome) === normalizeText(name))
...
function findMember(members: TeamMember[], name: string) {
  ...
  if (!member) throw new Error(`Membro "${name}" nao encontrado na equipe de Louvor.`)
  return member
}
...
function buildMembersForTeam(
  ...
  preset.members.forEach(item => {
    const member = members.find(memberItem => memberItem.id === item.team_member_id)
    const fn = functions.find(functionItem => functionItem.id === item.function_id)
  ...
  if (
    ...
    throw new Error('Validacao: Nilson nao pode tocar bateria e baixo no mesmo dia.')
  }
...
function identifyPresetFromSchedule(schedule: Schedule, presets: WorshipFixedTeam[]) {
  ...
  return presets.find(preset =>
    ...
    preset.members.every(member => schedulePairs.has(`${member.team_member_id}:${member.function_id}`))
  )
}

function identifyFunctionMember(schedule: Schedule, functionName: string) {
  return (schedule.members || []).find(member =>
    (member.functions || []).some(fn => normalizeText(fn.nome) === normalizeText(functionName))
  )?.team_member?.user?.nome
}

async function loadContinuityState(teamId: string, beforeDate: string, presets: WorshipFixedTeam[]) {
  ...
  const schedules = (data || []).map((schedule: any) => ({
    ...
    members: (schedule.members || []).map((member: any) => ({
      ...member,
      functions: (member.functions || []).map((fn: any) => fn.function).filter(Boolean),
    })),
  })) as Schedule[]
  ...
  for (const schedule of schedules) {
    if (!state.lastTeamCode) {
      const preset = identifyPresetFromSchedule(schedule, presets)
      const code = preset ? getTeamCode(preset.nome) : getTeamCode(schedule.title || '')
    ...
    if (state.lastTeamCode && state.lastDrummerName && state.lastBassistName) break
  }
...
export const worshipAutoScheduleService = {
  async generateMonthly(teamId: string, monthDate = new Date()): Promise<WorshipAutoScheduleResult> {
    const team = await teamService.getTeamById(teamId)
    if (!team.team_type_id) throw new Error('Equipe de Louvor sem tipo configurado.')

    const [presets, members, functions] = await Promise.all([
      worshipFixedTeamService.getByTeamId(teamId),
      teamService.getTeamMembers(teamId),
      ensureFunctions(team.team_type_id, ['Vocal', 'BackVocal', 'Bateria', 'Baixo', 'Teclado', 'Guitarra']),
    ])

    const presetMap = buildPresetMap(presets)
    const missingPresets = REQUIRED_PRESET_CODES.filter(code => !presetMap.has(code))
    if (missingPresets.length > 0) {
      throw new Error(`Cadastre as equipes padrao antes de gerar: ${missingPresets.join(', ')}.`)
    }
    ...
    const continuity = await loadContinuityState(teamId, monthStartText, presets)

    const existingSchedules = await scheduleService.getSchedulesByMonth(teamId, monthDate)
    const existingDates = new Set(existingSchedules.map(schedule => schedule.date))
    ...
    for (const [blockIndex, blockDates] of weekendBlocks.entries()) {
      const teamCode = blockIndex === 0
        ? FIRST_WEEKEND_TEAM_CODE
        : nextFromCycle(TEAM_SEQUENCE, lastTeamCode)
      const preset = presetMap.get(teamCode)!

      const drummerName = teamCode === FIRST_WEEKEND_TEAM_CODE
        ? 'Nilson'
        : nextFromCycle(DRUMMER_SEQUENCE, lastDrummerName)

      let bassistName = teamCode === FIRST_WEEKEND_TEAM_CODE || drummerName === 'Thiago'
        ? 'Daniel'
        : nextFromCycle(BASS_SEQUENCE, lastBassistName)

      if (bassistName === 'Nilson' && drummerName === 'Nilson') {
        bassistName = nextFromCycle(BASS_SEQUENCE, bassistName)
      }
