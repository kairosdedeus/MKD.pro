...
export const scheduleService = {
  ...
  async createSchedule(scheduleData: ScheduleFormData) {
    ...
    if (scheduleError) {
      if (scheduleError.code === '23505') {
        throw new Error(`Já existe uma escala para esta equipe no dia ${scheduleData.date}. Edite a escala existente ou escolha outra data.`)
      }
      throw scheduleError
    }
  ...
  async updateSchedule(scheduleId: string, scheduleData: Partial<ScheduleFormData>) {
    const { data: authData } = await supabase.auth.getUser()
    const { data: userProfile } = await supabase
      .from('users_profile')
      .select('id')
    ...
    if (scheduleData.members !== undefined) {
      ...
      if (existingMembers && existingMembers.length > 0) {
        ...
        await supabase.from('schedule_member_functions').delete().in('schedule_member_id', memberIds)
      }
      ...
      for (const member of scheduleData.members) {
        ...
        if (member.function_ids.length > 0) {
          ...
          const { error: fnError } = await supabase.from('schedule_member_functions').insert(functions)
    ...
    if (scheduleData.songs !== undefined) {
      ...
      if (scheduleData.songs.length > 0) {
        ...
        const { error: songsError } = await supabase.from('schedule_songs').insert(songs)
        if (songsError) throw songsError
      }
    }
