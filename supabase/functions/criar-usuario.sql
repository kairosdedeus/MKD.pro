-- ============================================
-- FUNÇÃO: create_user_with_auth
-- Cria usuário no auth.users + users_profile
-- Execute uma vez para registrar a função
-- ============================================
...
BEGIN
  ...
  VALUES (
    ...
    FALSE, NOW(), NOW(), NULL, NULL, '', '', NULL, '', 0,
  ...
  VALUES (
    v_auth_user_id::text, gen_random_uuid(), v_auth_user_id,
    jsonb_build_object(
      'sub', v_auth_user_id::text, 'email', p_email,
  ...
  -- 3. Criar perfil
  INSERT INTO users_profile (auth_user_id, nome, email, telefone, ativo)
  VALUES (v_auth_user_id, p_nome, p_email, p_telefone, TRUE)
...
SELECT '✅ Função create_user_with_auth registrada!' as resultado;
